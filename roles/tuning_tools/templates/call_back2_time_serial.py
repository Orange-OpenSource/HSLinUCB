...
try:
   ...
   print("Usage: ./" + sys.argv[0] + " ExpectedResponseTime IterationNumber")
   print("ResponseTime: Expected response time in milliseconds")
...
while loop:
   ...
   try:
      ...
      url = "http://{{hostvars[groups['master'][0]].ansible_default_ipv4.address}}:{{ingress_gateway_port}}/code/Proxy2BackSerial?path1=ResponseTime?time=" + sys.argv[1] + "&path2=ResponseTime?time=" + sys.argv[1]
