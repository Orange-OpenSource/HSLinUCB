...
class OrchestratorActionResult(Thread):
    def __init__(self, debug=False):
        ...
        self.debug=debug
        self.OrchestratorActionResult_consumer = KafkaConsumer('action_result',bootstrap_servers=['{{hostvars[groups['master'][0]].ansible_default_ipv4.address}}:{{kafka_component.spec.ports[0].port}}'],value_deserializer=lambda x: loads(x.decode('utf-8')))
        self.resultTable = {}
...
class StateResult(Thread):
    def __init__(self, debug=False):
        Thread.__init__(self)
        self.debug=debug
        self.StateResult_consumer = KafkaConsumer('state',bootstrap_servers=['{{hostvars[groups['master'][0]].ansible_default_ipv4.address}}:{{kafka_component.spec.ports[0].port}}'],value_deserializer=lambda x: loads(x.decode('utf-8')))
        self.stateTable = {}
...
class Orchestrator():
    def __init__(self,debug=False, sessionFile=None):
        self.debug=debug
        self.kafka_producer = KafkaProducer(bootstrap_servers = ['{{hostvars[groups['master'][0]].ansible_default_ipv4.address}}:{{kafka_component.spec.ports[0].port}}'],acks='all',value_serializer=lambda x: dumps(x).encode('utf-8'))
